...
> Видно то, на что есть ссылка, остальное - мусор
...
- `git reflog <ref>` - показать лог действий со ссылкой
- `git reflog` = `git reflog show HEAD` - показать лог действий с HEAD
- `<ref>~N` = N-й родитель от N-го поколения, например, `HEAD~3`
- `<ref>^N` = N-й родитель предыдущего поколения, например, `HEAD~1^2`
- `<ref>@{N}` = N-й предшественник по reflog, например, `HEAD@{5}`
